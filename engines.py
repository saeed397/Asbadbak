...
# ------------------------------------------------------------------
# 5) Funding Engine (Funding Rate / Open Interest)
# ------------------------------------------------------------------
# رفع باگ: صرافی/نماد اسپات (مثلاً exchange در حالت پیش‌فرض + "BTC/USDT")
# اصلاً فاندینگ ندارد؛ فاندینگ فقط در بازار پرپچوال (Swap) با نماد یکپارچه‌ی
# ccxt به‌صورت "BTC/USDT:USDT" وجود دارد. نسخه‌ی قبلی همیشه exceptions
# می‌گرفت (چون نماد/نوع بازار اشتباه بود) و به مقدار خنثی ۵۰ سقوط می‌کرد.
# اصلاح: یک نمونه‌ی جداگانه از همان صرافی با options.defaultType="swap"
# ساخته و کش می‌شود، و نماد اسپات به نماد پرپچوال معادل تبدیل می‌شود.
_FUNDING_EXCHANGE_CACHE = {}
...
def _get_funding_exchange(exchange_id: str):
    if exchange_id in _FUNDING_EXCHANGE_CACHE:
        return _FUNDING_EXCHANGE_CACHE[exchange_id]
    ex = None
    try:
        import ccxt
        ...
        ex = None
    _FUNDING_EXCHANGE_CACHE[exchange_id] = ex
    return ex


def funding_engine(exchange, symbol: str) -> dict:
    """
    نرخ فاندینگ و بهره‌ی باز (Open Interest) بازار پرپچوال متناظر با نماد اسپات.
    ...
    اگر صرافی از بازار پرپچوال یا نماد پشتیبانی نکند، امتیاز خنثی (۰.۰)
    برمی‌گرداند تا در میانگین وزنی اثر مصنوعی نگذارد.
    """
    neutral = {"funding_score": 0.0, "funding_rate": None, "open_interest": None, "available": False}
    try:
        swap_ex = _get_funding_exchange(getattr(exchange, "id", None))
        if swap_ex is None:
            return neutral
        swap_symbol = _to_swap_symbol(symbol)
        markets = getattr(swap_ex, "markets", None)
        if markets and swap_symbol not in markets:
            return neutral
        if not getattr(swap_ex, "has", {}).get("fetchFundingRate", False):
            return neutral
        data = swap_ex.fetch_funding_rate(swap_symbol)
        ...
        open_interest = None
        if getattr(swap_ex, "has", {}).get("fetchOpenInterest", False):
            try:
                oi_data = swap_ex.fetch_open_interest(swap_symbol)
