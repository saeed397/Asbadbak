...
def analyze_symbol(exchange, symbol: str, cfg: dict):
    ...
    # ---------- الزام تأیید صریحِ تایم‌فریم بالاتر برای همه‌ی سیگنال‌ها ----------
    # طبق قانون پروژه، فقط سیگنال‌هایی که با تایم‌فریم بالاتر «موافق» هستند
    # به‌عنوان خروجی نهایی اعلام می‌شوند. سیگنال «مخالف» یا سیگنالی که داده‌ی
    # تأییدی برایش در دسترس نبود (مثلاً تایم‌فریم اصلی از قبل بالاترین سطح
    # سلسله‌مراتب بود)، هر دو حذف می‌شوند — نه فقط موارد «مخالف». این یکسان‌سازی
    # باعث می‌شود خط «موافق روند تایم‌فریم ...» زیر تمام سیگنال‌های نمایش داده
    # شده، بدون استثنا، درج شود.
    if result["signal"] and result["confirm_status"] != "agree":
        result["signal"] = None
...
def auto_scan_step(cfg: dict, start_pos: int, batch_size: int = 50, max_rank: int = 500, max_batches: int = None):
    """
    ...
    max_batches (اختیاری): سقف تعداد بلوک‌های ۵۰تایی که در یک فراخوانی
    بررسی می‌شوند، حتی اگر سیگنالی پیدا نشده باشد؛ این یک محافظ اجرایی است
    (نه تغییری در منطق استراتژی) تا در نادرترین حالت که هیچ سیگنالی در چند
    بلوک پیاپی پیدا نمی‌شود، یک فراخوانی بیش از حد طولانی/سنگین نشود. با زدن
    مجدد دکمه، جستجو دقیقاً از همان‌جا ادامه می‌یابد.
