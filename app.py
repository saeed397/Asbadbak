...
# ---------------------------------------------------------------------------
# جستجوی خودکار پلکانی (۵۰ به ۵۰): هر بار ۵۰ رمزارز بعدی (طبق مارکت‌کپ) در
# تایم‌فریم انتخاب‌شده بررسی می‌شود؛ به محض یافتن اولین سیگنال متوقف و نمایش
# داده می‌شود. با زدن مجدد، دقیقاً از همان‌جا (نه از ابتدا) ادامه می‌یابد، تا
# سیگنال جدید پیدا شود یا کل ۵۰۰ رمزارز اول مارکت‌کپ بررسی شود.
# ---------------------------------------------------------------------------
st.divider()
st.subheader("🔁 جستجوی خودکار پلکانی")
st.caption(f"هر بار ۵۰ رمزارز بعدی (بر اساس مارکت‌کپ، تا سقف {MAX_MARKET_CAP_RANK}) در تایم‌فریم انتخاب‌شده "
           "بررسی می‌شود و به محض یافتن اولین سیگنال، جستجو متوقف می‌شود. با زدن مجدد دکمه، جستجو از همان نقطه ادامه می‌یابد.")
...
auto_cols = st.columns([3, 1])
with auto_cols[0]:
    auto_run = st.button("🔎 جستجوی خودکار (۵۰ رمزارز بعدی)", use_container_width=True)
with auto_cols[1]:
    auto_reset = st.button("↺ شروع مجدد", use_container_width=True, disabled=(st.session_state[_scan_key] <= 1))
...
if auto_run:
    ...
    if start_pos > MAX_MARKET_CAP_RANK:
        ...
        with st.spinner(f"در حال جستجوی پلکانی از رتبه‌ی {start_pos}..."):
            found, next_pos, scan_complete = auto_scan_step(auto_cfg, start_pos, batch_size=50, max_rank=MAX_MARKET_CAP_RANK)
